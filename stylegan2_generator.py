    ...
    def build(self, input_shape):
        ...
        for i in range(self.mapping_layers):
            setattr(self, 'Dense{}'.format(i),
                    DenseLayer(fmaps=256, lrmul=self.lrmul, name='Dense{}'.format(i)))

        self.Conditional_Dense = DenseLayer(fmaps=256, lrmul=self.lrmul, name='Conditional_Dense')
...
class StyleGan2Generator(tf.keras.layers.Layer):
    ...
    def __init__(self, resolution=1024, weights=None, impl='cuda', gpu=True, **kwargs):
        ...
        if weights is not None:
            ...
            _ = self(tf.zeros(shape=(1, 256)), lambda_t=1, c=tf.zeros(shape=(1, num_classes)))
